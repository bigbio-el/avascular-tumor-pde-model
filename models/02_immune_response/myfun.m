...
function [F]= myfun(x,DX,Dc,Dl,alpha,gamma,k1,k2,k3,k4,k5,k6,lv,c0,N)
   ...
   for i = 1 : (N)       
     ...
     % Forward Finite Difference Approximation
      ...
      if i == 1
        ...
        dp(i,1) = ((P1_l - P1)/DX) * ((PQR_l - PQR)/DX) + P1 * ((PQR_l - 2 * PQR + PQR_l)/DX^2) + G(x(i + 4*N,1)) * x(i,1) * (1 - PQNS_s) - Z(x(i + 4*N,1)) * x(i,1) - k5 * x(i,1) * x(i + 5*N,1);
        ...
        dl(i,1) = Dl *(x(i + 5*N + 1,1) - 2 * x(i + 5*N,1) + x(i + 5*N + 1,1))/(DX^2) - lv * (x(i + 5*N + 1,1) - x(i + 5*N,1))/DX - k4 * x(i,1) * x(i + 5*N,1) + G(x(i + 4*N,1)) * x(i + 5*N,1) * (gamma - PQNS_s);
     ...
     % Central Finite Difference Approximation
      ...
      if ((i > 1) && (i < N))
         ...
         dp(i,1) = ((P1_l - P1_r)/(DX*2)) * ((PQR_l - PQR_r)/(DX*2)) + P1 * ((PQR_l - 2 * PQR + PQR_r)/DX^2) + G(x(i + 4*N,1)) * x(i,1) * (1 - PQNS_s) - Z(x(i + 4*N,1)) * x(i,1) - k5 * x(i,1) * x(i + 5*N,1);
         ...
         dl(i,1) = Dl *(x(i + 5*N + 1,1) - 2 * x(i + 5*N,1) + x(i + 5*N - 1,1))/(DX^2) - lv * (x(i + 5*N + 1,1) - x(i + 5*N - 1,1))/(DX*2) - k4 * x(i,1) * x(i + 5*N,1) + G(x(i + 4*N,1)) * x(i + 5*N,1) * (gamma - PQNS_s);
     ...
     % Backward Finite Difference Approximation
      ...
      if i == N
         ...
         dp(i,1) = ((P1 - P1_r)/DX) * ((PQR - PQR_r)/DX) + P1 * ((PQR_r - 2 * PQR + PQR_r)/DX^2) + G(x(i + 4*N,1)) * x(i,1) * (1 - PQNS_s) - Z(x(i + 4*N,1)) * x(i,1) - k5 * x(i,1) * x(i + 5*N,1);
         ...
         dl(i,1) = Dl *(x(i + 5*N - 1,1) - 2 * x(i + 5*N,1) + x(i + 5*N - 1,1))/(DX^2) - lv * (x(i + 5*N,1) - x(i + 5*N - 1,1))/DX - k4 * x(i,1) * x(i + 5*N,1) + G(x(i + 4*N,1)) * x(i + 5*N,1) * (gamma - PQNS_s);
